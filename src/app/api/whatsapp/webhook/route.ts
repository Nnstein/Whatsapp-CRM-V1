...
async function processMessage(
  ...
  const contentType = ALLOWED_CONTENT_TYPES.has(message.type)
    ...
    : message.type === 'sticker'
      ...
      : message.type === 'button'
        ? 'interactive' // quick-reply button clicks are interactive replies
  ...
  // flow already consumed the message.
  ...
  // handoff layer didn't already handle it, and only when the account
  // has enabled it. `dispatchInboundToAiReply` owns its eligibility gates
  // + try/catch and never throws.
  if (!flowConsumed && !interactiveReplyId && !handoffHandled && inboundText.trim()) {
...
async function parseMessageContent(
  ...
  switch (message.type) {
    ...
    case 'button': {
      ...
      // Meta delivers `message.button` containing `text` (button label) and `payload`.
      const btn = message.button
      const buttonText = btn?.text || btn?.payload || '[Button reply]'
      return {
        ...empty,
